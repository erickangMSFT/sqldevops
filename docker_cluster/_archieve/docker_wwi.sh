...
docker run --name mssql -e 'ACCEPT_EULA=Y' -e 'SA_PASSWORD=SqlDevOps2017' -p 1433:1433 -d microsoft/mssql-server-linux  
docker exec -ti mssql mkdir /var/opt/mssql/backup
cp ~/dockershare/WorldWideImporters2017.bak mssql:/var/opt/mssql/backup
sqlcmd -Usa -PSqlDevOps2017 -i restore_wwi.sql 
sqlcmd -Usa -P SqlDevOps2017 -Q'select name from sys.databases'
...
docker run --name sqldev -e 'ACCEPT_EULA=Y' -e 'SA_PASSWORD=SqlDevOps2017' -p 1433:1433 -d sqldevopsacs.azurecr.io/mssql:20170806-2 /opt/mssql/bin/sqlservr
